...
| CTRCs marcados `departed` via 930 | ~4.519 |
...
**Decisao arquitetural mais importante.** Na implementacao inicial (0001), saidas eram marcadas como `verification_status = 'departed'`. Problema identificado: isso sobrescrevia o historico de verificacao. Um CTRC que foi verificado perdia esse registro ao ser marcado como departed.
...
Isso permite cruzamentos como: "dos CTRCs verificados, quantos foram entregues?" sem perder dados.
...
**Fix**: trocar para f-string com codigos expandidos.

### 6.2 Filtro por date vs timestamp

O filtro `b.data_ocor >= lv.arrived_at::date` permitia pegar eventos de saida anteriores a chegada no mesmo dia (ex: saida as 04:39, chegada as 23:24 — carga que passou 2x pela mesma unidade). Resultou em 755 rows com `departed_at < arrived_at`.
...
|-----------------|-----------|----------|
| Chegada na unidade (recente) | ~500+ | Sim — carga recente |
| Aguardando autorizacao/devolucao | 628 | Sim — retida |
| Processo arquivado | 334 | Sim — finalizado |
| Aguardando agendamento | ~576 | Sim — esperando |
| Falta com emissao de NFD | 110 | Sim — problema registrado |
| Saida da unidade (455 desatualizada) | ~130 | Bug residual da 455 |
| Pendencia financeira/debito | ~100+ | Sim — retida |
